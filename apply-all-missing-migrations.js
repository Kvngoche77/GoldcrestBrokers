/**
 * apply-all-missing-migrations.js
 *
 * Generates the complete SQL migration file for all missing copy-trading
 * columns and tables, then outputs it for copy-paste into Supabase SQL Editor.
 *
 * Run with:  node apply-all-missing-migrations.js
 */

const fs = require('fs');
const path = require('path');

const OUTPUT_FILE = path.join(__dirname, 'supabase', 'migrations', '20260814_fix_all_copy_trading.sql');
...
console.log('\n✅  SQL migration file written to:');
console.log(`    ${OUTPUT_FILE}\n`);
console.log('═══════════════════════════════════════════════════════════════════');
console.log('  NEXT STEP – Apply to Supabase in 3 easy steps:');
console.log('');
console.log('  1. Open: https://supabase.com/dashboard/project/ixqcnzmhgdysznjaghfx/sql/new');
console.log('  2. Copy the contents of the file above and paste into the editor.');
console.log('  3. Click "Run" (or press Ctrl+Enter).');
console.log('');
console.log('  All columns, tables, RLS policies and RPCs will be created/updated.');
console.log('  The script uses IF NOT EXISTS everywhere so it is fully idempotent.');
console.log('═══════════════════════════════════════════════════════════════════\n');

// Also print a summary of what will be created
console.log('Summary of changes:');
console.log('  • copy_traders            +10 columns (total_active_days, currency, leverage,');
console.log('                             platform, account_type, risk_score, max_drawdown,');
console.log('                             equity, performance_history, strategy_history)');
console.log('  • copy_trading_subscriptions +5 columns (expires_at, days_credited,');
console.log('                             last_credited_at, daily_roi_percent, last_processed_at)');
console.log('  • copy_trader_positions    NEW table + RLS policies + indexes');
console.log('  • copy_trader_applications NEW table + RLS policies + indexes');
console.log('  • process_copy_trading_daily_5pct_returns  NEW/UPDATED RPC');
console.log('  • simulate_global_trader_activity          NEW/UPDATED RPC');
console.log('  • process_user_copy_trades                 NEW/UPDATED RPC');
